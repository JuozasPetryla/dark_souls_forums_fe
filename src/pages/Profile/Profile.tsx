...
export default function Profile() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const res = await callAPI<User>(endpoint, true, { method: "GET" });
  ...
  async function connectSteam() {
    ...
    const res = await callAPI("/profile/steam/add", true, {
      method: "POST",
      body: steamId,
    });
  ...
  return (
    <div className="max-w-4xl mx-auto mt-12 space-y-10">
      ...
      <h1 className="text-4xl font-bold text-center tracking-tight">
        {isMyProfile ? "Mano profilis" : `Naudotojo profilis: ${user.nickname}`}
      </h1>
      ...
      <div className="bg-base-100 p-8 rounded-2xl shadow-md border border-base-300 space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="text-sm opacity-70">
            Paskutinį kartą prisijungė: {formatDate(user.last_login_at)}
          </div>

          {user.bio && (
            <p className="mt-2 leading-relaxed">
              <strong>Aprašymas:</strong> {user.bio}
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          <p><strong>Vardas:</strong> {f(user.name)}</p>
          <p><strong>Pavardė:</strong> {f(user.surname)}</p>
          <p><strong>Šalis:</strong> {f(user.origin_country)}</p>
          <p><strong>Miestas:</strong> {f(user.city)}</p>

          <p><strong>Telefono nr.:</strong> {f(user.phone_number)}</p>
          <p><strong>Discord:</strong> {f(user.discord_id)}</p>
          <p><strong>Steam ID:</strong> {f(user.steam_id)}</p>

          <p><strong>Klasė žaidime:</strong> {f(user.playing_class)}</p>

          <p>
            <strong>Adresas:</strong>{" "}
            {`${f(user.address)}, ${f(user.city)}, ${f(user.postal_code)}`}
          </p>
          ...
          <dialog open className="modal modal-open">
            <div className="modal-box space-y-6">
              ...
              <div className="p-4 rounded-lg bg-base-200 text-sm space-y-1">
                <p><strong>Slapyvardis:</strong> {user.nickname}</p>
                <p><strong>Paskutinį kartą prisijungė:</strong> {formatDate(user.last_login_at)}</p>
              </div>
              ...
              <div className="modal-action flex justify-end gap-3">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    const res = await callAPI("profiles/delete", true, { method: "DELETE" });
                ...
                <>
                  ...
                  <div className="modal-action flex justify-end gap-3">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        const res = await callAPI("profiles/profile/steam/add", true, {
                          method: "POST",
                          body: steamInput.trim(),
                        });
